...
const st2 = @import("SumTree2.zig");
const SumTree2 = st2.SumTree2;
...
fn collectLeafSlices2(allocator: std.mem.Allocator, node: anytype, chunks: []const u8, bucket: *std.ArrayList(u8)) anyerror!void {
    if (node.isLeaf()) {
        const len = node.summary.dimensions[0];
        try bucket.appendSlice(allocator, chunks[node.start .. node.start + len]);
    } else {
        for (node.children.slice()) |child| {
            try collectLeafSlices2(allocator, child, chunks, bucket);
        }
    }
}

test "SumTree2 basic functionality" {
    const allocator = std.heap.page_allocator;
    const S = SumTree2(u8);
    ...
    defer bucket.deinit(allocator);
    try collectLeafSlices2(allocator, sliced.root, sliced.chunks.items, &bucket);
